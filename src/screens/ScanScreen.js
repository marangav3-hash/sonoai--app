  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      ...
      <TouchableOpacity style={s.picker} onPress={pickFile}>
        {file
          ? (<><Text style={s.fileName}>{file.name}</Text><Text style={s.fileSize}>{(file.size/1024).toFixed(1)} KB</Text></>)
          : (<><Text style={s.pickerIcon}>+</Text><Text style={s.pickerText}>Tap to select DICOM file</Text><Text style={s.pickerSub}>.dcm files only</Text></>)
        }
      </TouchableOpacity>
      ...
      {uploading ? (
        ...
        <TouchableOpacity style={[s.btn, !file && { opacity: 0.4 }]} onPress={handleUpload} disabled={!file}>
...
const s=StyleSheet.create({
  container:{flex:1,backgroundColor:"#F7F7F5"},content:{padding:20},
  label:{fontSize:11,fontWeight:"700",color:"#888",letterSpacing:2,marginBottom:10,marginTop:20},
  picker:{backgroundColor:"#fff",borderRadius:14,borderWidth:2,borderColor:"#1D9E75",borderStyle:"dashed",padding:32,alignItems:"center",gap:6},
  pickerIcon:{fontSize:32,color:"#1D9E75"},pickerText:{fontSize:15,fontWeight:"600",color:"#085041"},pickerSub:{fontSize:13,color:"#888"},
  fileName:{fontSize:15,fontWeight:"600",color:"#085041",textAlign:"center"},fileSize:{fontSize:13,color:"#888"},
  chips:{flexDirection:"row",flexWrap:"wrap",gap:8},
  chip:{paddingHorizontal:14,paddingVertical:8,borderRadius:999,borderWidth:1,borderColor:"#ddd",backgroundColor:"#fff"},
  chipOn:{backgroundColor:"#085041",borderColor:"#085041"},chipText:{fontSize:13,color:"#555"},chipTextOn:{color:"#fff",fontWeight:"600"},
  ...
  uploadingBox:{alignItems:"center",padding:32,gap:12},stage:{fontSize:14,color:"#085041",fontWeight:"500"},
  btn:{backgroundColor:"#085041",borderRadius:14,padding:18,alignItems:"center",marginTop:24},btnText:{color:"#fff",fontSize:16,fontWeight:"700"},
});
